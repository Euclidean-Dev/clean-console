...
CleanConsole.init(configOptions) // Object parameter for custom options
```
...
To customize the behaviour you can pass a config object with the following properties. All properties are optional:
...
| `debugLocalStorageKey` |   string   |          | Set the local storage key that you want to use to block the execution of the library and be able to see all logs for debugging | `debugLocalStorageKey: 'my_awesome_site_key'` |
